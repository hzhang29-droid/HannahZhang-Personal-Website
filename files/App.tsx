...
const INFJ_FRONT: Record<string, string> = { I: "Introverted", N: "Intuitive", F: "Feeling", J: "Judging" };
const INFJ_BACK: Record<string, string> = {
  I: "notices the room before she speaks in it",
  N: "feels it first, explains it later",
  F: "keeps score quietly, mostly with herself",
  J: "needs a rhythm of her own",
};

const MARGIN_NOTES = [
  "能感受到很多情绪，和大家在一起时会顾全大局，但常常把自己掏空。",
  "会暗暗和身边人比一比，权衡一下自己的位置。",
  "需要 inner peace，需要及时充电。",
  "讨厌无营养的 small talk。",
  "想成为能带别人走上正轨的人，也想要那种成就感。",
];
...
// ─── InfjFlip — surface label vs. what's actually true, revealed on hover ─────
function FlipCard({ letter }: { letter: string }) {
    ...
    >
      <div style={{
        ...
        <div style={{
          ...
          border: `1px solid ${C.ink}`, background: C.bone,
          fontFamily: SERIF, fontSize: 25, fontWeight: 500, color: C.ink,
        }}>
          {letter}
          <span style={{ font: `500 7px/1 ${MONO}`, letterSpacing: "0.05em", textTransform: "uppercase", color: "#9b9890", marginTop: 4 }}>
            {INFJ_FRONT[letter]}
          </span>
        </div>
        <div style={{
          ...
          background: C.ink, color: C.bone, padding: "5px 6px",
          fontFamily: SERIF, fontStyle: "italic", fontSize: 10.5, lineHeight: 1.25,
        }}>
          {INFJ_BACK[letter]}
        </div>
      </div>
...
function InfjFlip() {
  return (
    <div style={{ display: "flex", gap: 9, marginTop: 20, perspective: 900 }}>
      {["I", "N", "F", "J"].map(l => <FlipCard key={l} letter={l} />)}
    </div>
  );
}

// ─── Marginalia — loose notes in her own words, not a resume section ──────────
function MarginNote({ text, index }: { text: string; index: number }) {
  const [h, setH] = useState(false);
  const rotate = [-3, 2, -1.5, 2.5, -2][index % 5];
  const mt = [0, 14, 0, 20, 6][index % 5];
  return (
    <div
      ...
      style={{
        ...
        background: "#fffdf2", padding: "18px 16px 14px",
        boxShadow: h ? "0 14px 26px rgba(0,0,0,0.18)" : "0 8px 18px rgba(0,0,0,0.12)",
        fontFamily: SERIF, fontStyle: "italic", fontSize: 15.5, lineHeight: 1.4,
        color: "#3a3323",
        transform: h ? "translateY(-4px) rotate(0deg)" : `rotate(${rotate}deg)`,
        marginTop: mt,
    ...
    >
      <span style={{
        ...
        width: 28, height: 13, background: index % 2 === 0 ? "rgba(235,91,72,0.28)" : "rgba(239,181,194,0.5)",
      }} />
      {text}
    </div>
  );
...
function Marginalia() {
  return (
    <section style={{ background: C.bone, padding: "20px 0 90px" }}>
      <div style={{ ...wrap, display: "flex", flexWrap: "wrap", gap: 22, justifyContent: "center" }}>
        {MARGIN_NOTES.map((note, i) => <MarginNote key={i} text={note} index={i} />)}
      </div>
    </section>
  );
}

// ─── PortalFigure — hover to see the layer under the surface ─────────────────
function PortalFigure({ img, caption, rotate }: { img: string; caption: string; rotate: number }) {
  ...
  return (
    <figure
      ...
      style={{
        ...
        background: C.ink, border: "7px solid #fffaf0",
        boxShadow: `7px 8px 0 ${C.ink}`, transform: `rotate(${rotate}deg)`,
        gridRow: "span 2", transition: "transform 0.35s ease",
      }}
    >
      <img
        src={`https://images.unsplash.com/photo-${img}?w=600&h=420&fit=crop&auto=format`}
        alt={caption}
        style={{ width: "100%", height: "100%", objectFit: "cover", filter: "contrast(1.04) saturate(0.76)", display: "block" }}
      />
      {/* inner layer — only visible inside the cursor circle */}
      <div style={{
        position: "absolute", inset: 0, pointerEvents: "none",
        clipPath: pos ? `circle(72px at ${pos.x}px ${pos.y}px)` : "circle(0px at 50% 50%)",
        transition: pos ? "none" : "clip-path 0.3s ease",
      }}>
        <img
          src={`https://images.unsplash.com/photo-${img}?w=600&h=420&fit=crop&auto=format`}
          alt=""
          style={{ width: "100%", height: "100%", objectFit: "cover", filter: "sepia(0.45) saturate(1.6) contrast(1.1) brightness(0.9)", display: "block" }}
        />
        {pos && pos.x < 150 && pos.y > 250 && (
          <span style={{
            position: "absolute", left: 14, top: 226,
            fontFamily: SERIF, fontStyle: "italic", fontSize: 13, color: C.bone,
            textShadow: "0 1px 4px rgba(0,0,0,0.6)",
          }}>this is closer to how it feels</span>
        )}
      </div>
      <figcaption style={{
          ...
          <Reveal delay={0.15}>
            <div style={{
              ...
              {[
                ...
                img.span ? (
                  <div key={i} style={{ gridRow: "span 2" }}>
                    <PortalFigure img={img.src} caption={img.caption} rotate={img.rotate} />
                  </div>
                ) : (
                <figure key={i} style={{
                  position: "relative", margin: 0, overflow: "hidden",
